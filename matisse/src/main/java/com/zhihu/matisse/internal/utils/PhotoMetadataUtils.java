...
public final class PhotoMetadataUtils {
    ...
    public static Point getBitmapSize(Item item, Activity activity) {
        ContentResolver resolver = activity.getContentResolver();
        Point imageSize = getBitmapBound(resolver, item.getContentUri());
        int w = imageSize.x;
        int h = imageSize.y;
        if (PhotoMetadataUtils.shouldRotate(resolver, item)) {
            w = imageSize.y;
            h = imageSize.x;
        }
    ...
    private static boolean shouldRotate(ContentResolver resolver, Item uri) {
        ExifInterface exif;
        try {
            exif = ExifInterfaceCompat.newInstance(getPath(resolver, uri.getContentUri()));
        } catch (IOException e) {
