...
public class AlbumMediaLoader extends CursorLoader {
    ...
    private static final String SELECTION_ALL_FOR_SINGLE_MEDIA_TYPE_VIDEO =
            MediaStore.Files.FileColumns.MEDIA_TYPE + "=?"
                    + " AND " + MediaStore.MediaColumns.SIZE + ">0"
            ;

    private static final String SELECTION_ALL_FOR_SINGLE_MEDIA_TYPE_IMAGE =
            MediaStore.Files.FileColumns.MEDIA_TYPE + "=?"
                    + " AND " + MediaStore.MediaColumns.SIZE + ">0"
    ...
    public static CursorLoader newInstance(Context context, Album album, boolean capture) {
        ...
        if (album.isAll()) {
            if (SelectionSpec.getInstance().onlyShowImages()) {
                selection = SELECTION_ALL_FOR_SINGLE_MEDIA_TYPE_IMAGE;
                selectionArgs = getSelectionArgsForSingleMediaType(MediaStore.Files.FileColumns.MEDIA_TYPE_IMAGE);
            } else if (SelectionSpec.getInstance().onlyShowVideos()) {
                selection = SELECTION_ALL_FOR_SINGLE_MEDIA_TYPE_VIDEO;
