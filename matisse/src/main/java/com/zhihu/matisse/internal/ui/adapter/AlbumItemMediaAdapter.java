...
public class AlbumItemMediaAdapter extends
        RecyclerViewItemAdapter<RecyclerView.ViewHolder> implements
    ...
    public AlbumItemMediaAdapter(Context context, SelectedItemCollection selectedCollection, RecyclerView recyclerView, boolean isVideo) {
        super(context,null);
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_CAPTURE) {
                ...
                public void onClick(View v) {
                    if (v.getContext() instanceof OnPhotoCapture) {
                        if (ContextCompat.checkSelfPermission(v.getContext(), Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
                            ...
                            onRequestPermissionsListener.onRequestPermissions(new String[]{Manifest.permission.CAMERA});
            ...
            return new MediaViewHolder(v, isVideo, MimeType.ofNotGif().containsAll(mSelectionSpec.mimeTypeSet));
        }
        return null;
    }
    ...
    public void setOnRequestPermissionsListener(AlbumItemMediaAdapter.onRequestPermissionsListener onRequestPermissionsListener) {
    ...
    public interface onRequestPermissionsListener {
         ...
         * @param strings 权限
         */
        void onRequestPermissions(String[] strings);
    }

    @Override
    protected void onBindViewHolder(final RecyclerView.ViewHolder holder, Item item) {
    ...
    private void setCheckStatus(Item item, MediaGrid mediaGrid) {
        if (mSelectionSpec.countable) {
            ...
            if (checkedNum > 0) {
                ...
                if (mSelectedCollection.maxSelectableReached()) {   //当选中图片超过最大值是 样式置灰
    ...
    public void onClickVideo(ImageView thumbnail, Item item, RecyclerView.ViewHolder holder) {
        if (assertAddSelection(holder.itemView.getContext(), item)) {
            if (item.duration / 1000 < SelectionSpec.getInstance().videoMinSize) {
                Toast.makeText(context, "视频最小时长不得少于" + SelectionSpec.getInstance().videoMinSize + "秒", Toast.LENGTH_SHORT).show();
                return;
            }
            if (item.duration / 1000 > SelectionSpec.getInstance().videoMaxSize) {
                Toast.makeText(context, "视频最大时长不得大于" + SelectionSpec.getInstance().videoMaxSize + "秒", Toast.LENGTH_SHORT).show();
                return;
            }
            mSelectedCollection.add(item);
            if (mOnMediaClickListener != null)
                mOnMediaClickListener.onVideoClick();
        }
    }
    ...
    @Override
    public int getItemViewType(int position, Item cursor) {
        return cursor.isCapture() ? VIEW_TYPE_CAPTURE : VIEW_TYPE_MEDIA;
    }
    ...
    private static class MediaViewHolder extends RecyclerView.ViewHolder {
        ...
        MediaViewHolder(View itemView, boolean isVideo, boolean isGif) {
            super(itemView);
            mMediaGrid = (MediaGrid) itemView;
            mMediaGrid.setIsVideo(isVideo);
            if (isGif | isVideo)
                mMediaGrid.onGoneCheck();
        }
    }
